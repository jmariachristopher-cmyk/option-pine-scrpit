...
def build_pine_text(data, symbol, expiry) -> str:
    ...
    L.append(f'selected = input.string({default_s}, "Select Strike", options=[{", ".join(strikes)}])')
    L.append("")
    ...
    L.append("")
    L.append("// ── Data from CSV (exact values, no recalculation) ───────")
    ...
    L.append("")
    L.append("// ── Plot on last bar ─────────────────────────────────────")
    L.append("idx = array.indexof(strike_arr, selected)")
    L.append("")
    L.append("if barstate.islast and idx >= 0")
    L.append("    s = selected")
    L.append(f'    label.new(bar_index, high, "Expiry: {expiry}", xloc=xloc.bar_index, yloc=yloc.abovebar,')
    L.append("              style=label.style_label_down, color=color.new(color.yellow,60),")
    L.append("              textcolor=color.yellow, size=size.normal)")
